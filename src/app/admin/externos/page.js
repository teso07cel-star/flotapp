  ...
  return (
    <div className="space-y-10 animate-in fade-in slide-in-from-bottom-4 duration-500 pb-20">
      <div>
        ...
        <p className="text-gray-500 ">Panel de control de terceros y vehículos externos.</p>
      </div>
                ...
                return (
                  <tr key={v.id} className="hover:bg-slate-800/30 dark:hover:bg-gray-800/20 transition-colors group">
                    ...
                    <td className="p-5">
                      {insp ? (
                        <div>
                          <p className="text-xs font-bold text-slate-100 ">Mes: {insp.mes}/{insp.anio}</p>
                      ...
                      {lastLog ? (
                        <div>
                          ...
                          <div className="flex gap-1 mt-1">
                             <span className="text-[10px] bg-slate-800/50  px-2 rounded-md font-bold uppercase text-gray-500">{lastLog.frecuenciaRegistro || 'diario'}</span>
                    ...
                    <td className="p-5 pr-8 text-right">
                       <Link href={`/admin/vehicles/${v.id}`} className="inline-flex py-2 px-3 bg-slate-900/40 bg-[#0f172a] text-slate-100  border border-slate-700  hover:bg-black hover:text-white rounded-xl text-[10px] font-black uppercase tracking-widest transition-all shadow-sm">
                         Expediente
                       </Link>
                    </td>
                  </tr>
