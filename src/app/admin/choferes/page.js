...
export default async function ChoferesAdmin() {
  try {
    const [res, authRes] = await Promise.all([
      getAllChoferes().catch(e => ({ success: false, error: e.message })),
      getAutorizacionesPendientes().catch(e => ({ success: false, error: e.message }))
    ]);
                  ...
                  {choferes.length === 0 ? (
                    ...
                    <tr key={c.id} className="hover:bg-slate-800/30 dark:hover:bg-gray-800/20 transition-colors group">
                      <td className="p-5 pl-8 font-bold text-slate-100 flex items-center gap-2">
                         <Link href={`/admin/choferes/${c.id}`} className="hover:text-blue-400 hover:underline transition-colors flex items-center gap-2">
                           {c.nombre}
                           {c.passkeyId && (
    ...
    console.error("FATAL error in ChoferesAdmin:", err);
    return (
      <div className="p-10 bg-red-50 dark:bg-red-900/10 border border-red-200 dark:border-red-800 rounded-3xl">
        <h2 className="text-red-600 dark:text-red-400 font-bold uppercase mb-2">Falla Crítica de Sistema</h2>
        <p className="text-gray-600 font-mono text-sm">{err.message}</p>
        <p className="mt-4 text-xs text-gray-500 italic text-balance">Se ha activado el protocolo de emergencia. Contacte a soporte técnico.</p>
      </div>
    );
  }
}
