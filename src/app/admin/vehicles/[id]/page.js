...
export default async function VehicleDetails({ params }) {
  ...
  async function saveAction(formData) {
    "use server";
    const patenteVal = formData.get("patente");
    const payload = {
      patente: patenteVal ? patenteVal.toString().toUpperCase() : undefined,
  ...
  const vtvStr = safeDateString(vehiculo.vtvVencimiento);
  const seguroStr = safeDateString(vehiculo.seguroVencimiento);
  ...
  return (
        ...
        <div className="lg:col-span-1 space-y-8">
          <div className="bg-white dark:bg-gray-900 border border-gray-200 dark:border-gray-800 rounded-[2.5rem] p-8 shadow-2xl shadow-black/5">
            ...
            <form action={saveAction} className="space-y-6">
              ...
              <div>
                <label className="block text-[10px] font-black uppercase text-gray-400 mb-2 tracking-widest">Patente (Matrícula)</label>
                <input
                  name="patente"
                  type="text"
                  defaultValue={vehiculo.patente}
                  className="w-full bg-gray-50 dark:bg-gray-950 border border-gray-200 dark:border-gray-800 rounded-2xl px-5 py-4 outline-none focus:ring-2 focus:ring-blue-500 transition-all font-bold uppercase"
                  required
                />
              </div>
              <div>
    ...
    return (
      <div className="p-10 m-10 bg-red-100 border-4 border-red-600 rounded-2xl text-red-900">
        <h1 className="text-2xl font-black uppercase mb-4 tracking-tighter shadow-sm text-red-700">Error Interceptado</h1>
        <p className="font-bold mb-6 text-sm">Toma captura de pantalla de este código y envíamelo, esto nos dirá exactamente por qué AF668JR se rompe:</p>
        <pre className="bg-black text-green-400 p-6 rounded-xl overflow-x-auto text-xs font-mono shadow-xl border border-gray-800">
          {err.name || "Error"}
          {"\n"}
