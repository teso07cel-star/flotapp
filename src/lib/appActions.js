...
export async function getMonthlySummary(month, year) {
  ...
  try {
    ...
    const TACTICAL_COORDS = {
      ...
      "Chascomus": { lat: -35.5768, lng: -58.0096 },
      "Independencia": { lat: -38.0004, lng: -57.5562 },
      "Guemes": { lat: -38.0125, lng: -57.5401 },
      "San Nicolas": { lat: -33.3332, lng: -60.2185 },
      ...
      if (driverName !== "S/D") {
         ...
         if (Array.isArray(r.sucursales)) {
            r.sucursales.forEach(s => {
               ...
               // Add to Driver+Vehicle map
               if (!c.branchDetails.has(branchId)) {
                  c.branchDetails.set(branchId, { 
                  ...
                  c.branchDetails.get(branchId).visitas++;
               }

               // Add to global branch tracking
    ...
    const fleetSummaryArray = Array.from(fleetMap.values()).map(d => ({
        id: `${d.conductor}-${d.patente}`,
       ...
       // Sort by driver name first, then km
       if (a.conductor === b.conductor) return b.kmRecorridos - a.kmRecorridos;
       return a.conductor.localeCompare(b.conductor);
    });

    // Consolidar driverStats agrupando por driver (para la Fase 2)
    const driverStatsMap = new Map();
    fleetSummaryArray.forEach(row => {
       if (!driverStatsMap.has(row.conductor)) {
          driverStatsMap.set(row.conductor, {
       ...
       const ds = driverStatsMap.get(row.conductor);
       ds.totalTrips += row.visitasSucursales;
       ds.totalKm += row.kmRecorridos;
       if (!ds.vehicles.includes(row.patente)) ds.vehicles.push(row.patente);
       
       row.branchDetails.forEach(b => {
          if (!ds.branchDetails.has(b.id)) {
             ds.branchDetails.set(b.id, { ...b });
          } else {
             ds.branchDetails.get(b.id).visitas += b.visitas;
          }
       });
    });

    const finalDriverStats = Array.from(driverStatsMap.values()).map(ds => ({
       ...ds,
       branchDetails: Array.from(ds.branchDetails.values())
    })).sort((a,b) => b.totalKm - a.totalKm);

    const finalData = {
      summary: fleetSummaryArray, // Usado en Fase 1 y tiene 1 fila por (Driver, Vehicle)
