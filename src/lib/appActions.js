...
/**
 * Genera el resumen mensual consolidado para "El Libro".
 */
export async function getMonthlySummary(month, year) {
  try {
    if (!process.env.DATABASE_URL) return { success: true, data: { summary: [], totalFleetVisits: 0, mapBranches: [] } };
    ...
    const vehicleStats = {};
    const branchVisitCount = {};

    registros.forEach(r => {
      const vId = r.vehiculoId || 0;
      const patente = r.vehiculo?.patente || "SIN_PATENTE";
      ...
      if (!vehicleStats[vId]) {
        vehicleStats[vId] = { 
          ...
          kmMax: r.kmActual, 
          visitasSucursales: 0,
          ultimoConductor: r.nombreConductor || "S/D"
        };
      }
      
      if (r.kmActual) {
        if (vehicleStats[vId].kmMin === null || r.kmActual < vehicleStats[vId].kmMin) vehicleStats[vId].kmMin = r.kmActual;
        if (vehicleStats[vId].kmMax === null || r.kmActual > vehicleStats[vId].kmMax) vehicleStats[vId].kmMax = r.kmActual;
      }
      
      vehicleStats[vId].visitasSucursales += r.sucursales?.length || 0;
      vehicleStats[vId].ultimoConductor = r.nombreConductor || vehicleStats[vId].ultimoConductor;

      r.sucursales.forEach(s => {
        branchVisitCount[s.id] = (branchVisitCount[s.id] || 0) + 1;
      });
    });

    const summary = Object.values(vehicleStats).map(v => ({
      ...v,
      kmRecorridos: Math.max(0, (v.kmMax || 0) - (v.kmMin || 0)),
      totalSpent: 0 // Placeholder para gastos futuros
    }));

    // Obtener info de sucursales para el mapa
    const branchIds = Object.keys(branchVisitCount).map(id => parseInt(id));
    const sucursalesInfo = await getPrisma().sucursal.findMany({
      where: { id: { in: branchIds } }
    });

    const mapBranches = sucursalesInfo.map(s => ({
      id: s.id,
      nombre: s.nombre,
      lat: s.lat,
      lng: s.lng,
      visits: branchVisitCount[s.id] || 0
    }));

    const totalFleetVisits = Object.values(branchVisitCount).reduce((a, b) => a + b, 0);
